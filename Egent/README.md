...
Optional: Override the default model (GPT-5-mini) and set custom API base URL if needed:
```bash
export EGENT_MODEL=gpt-5
export OPENAI_API_BASE="http://your-custom-base-url/v1"
```

You may refer to the `.env.example` file for the expected environment variables and their usage (see `config.py` for details).
