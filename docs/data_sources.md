...
本项目的主线输入对象是 quasar spectra，DESI 是第一批公开数据源。基础字段为：
...
为了测试 agent 是否会错误处理地球附近的 H I 发射线、连续谱异常和边界吸收，已经用本地 `srt/data` 里的 HST/COS 光谱跑过小批量真实样本。这类样本不是 DESI 训练主线，但很适合暴露工具层问题：

- 低红移 H I 吸收体会把 Ly alpha 吸收线放到接近地球 H I 发射线的位置。
- 如果输入表里有 `CONTINUUM` 列，review packet 应优先使用它，而不是用 DESI 假设重建连续谱。
- 不应对任意 flux 单位强行套接近零的绝对 continuum floor，否则 COS 这类低 flux scale 数据会把归一化残差放大到不可用。
- 这类样本适合作为 regression / stress test，用来检查 agent 是否会保守遮住发射线、是否会过度 mask、是否会在核心窗口内继续补 source。

当前这些真实数据只作为本地开发测试来源，不提交进仓库，也不把 `outputs/` 里的实验图当作长期资产保留。
...
非 DESI 光谱也可以进入同一套 review packet，只要先转换为 `wavelength / flux / ivar / pipeline_mask`，并尽量保留原始 pipeline 的 continuum 字段为 `CONTINUUM`。
