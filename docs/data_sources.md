...
- 不抢 C IV MVP 的位置。
- 可作为第二个 doublet 任务，验证 schema 是否能跨离子泛化。
- 可以给 RL reward 提供更多结构类似但波长区域不同的样本。
...
第一阶段仍以 C IV doublet 为 MVP，因为它的双线约束更容易形成可程序化 validator。H I / DLA 作为第二阶段扩展保留，文档和 schema 设计时提前兼容：
